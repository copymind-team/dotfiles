...
touch "$WORKTREE_BASE/main/.env.local"
OUTPUT=$("$SCRIPTS_DIR/dev-worktree-up.sh" feat-alpha 2>&1) || true

assert_file_exists "worktree created" "$WORKTREE_BASE/feat-alpha"
assert_file_exists "has supabase config" "$WORKTREE_BASE/feat-alpha/supabase/config.toml"
...
# Generated files
assert_file_exists ".env exists" "$WORKTREE_BASE/feat-alpha/.env"
assert_contains "COMPOSE_PROJECT_NAME" "COMPOSE_PROJECT_NAME=" "$(cat "$WORKTREE_BASE/feat-alpha/.env")"
assert_file_exists "override.yml exists" "$WORKTREE_BASE/feat-alpha/docker-compose.override.yml"
assert_contains "override has port mapping" ":3000" "$(cat "$WORKTREE_BASE/feat-alpha/docker-compose.override.yml")"
