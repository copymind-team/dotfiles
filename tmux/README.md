...
`.tmux.conf` plus three features built on top of it, each one script and a
keybinding: a [session picker](#session-picker), a [session monitor](#session-monitor)
and [session restore](#session-restore).

The first two read what Claude Code exports about itself — see
[claude/](../claude/README.md) for the exporters and how they are installed.
The [root README](../README.md#tmux) has the short version of all three.
